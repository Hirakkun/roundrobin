...
// 自動組合せトリガー（重複防止ガード付き）
// _scoresRef と _fbApply の両方から呼び出すことで B3（フォールバック）も担保
...
window._fbApply = function(remoteState) {
    ...
    try {
        ...
        if (state.roundCount > 0) {
            ...
            renderPlayerList();
            // B3フォールバック: _scoresRef が先に alreadyDone=true を設定してしまった場合でも
            // _fbApply 受信時に全コート done 検出で自動組合せをリトライ（_tryAutoGenerate の重複防止ガードにより二重生成しない）
            if (isAdmin && state.autoMatch && !state.seqMatch) {
                const lastRd = state.schedule[state.schedule.length - 1];
                if (lastRd) {
                    const allDone = lastRd.courts.every((ct, ci) =>
                        !!state.scores?.['r' + lastRd.round + 'c' + ci]?.done
                    );
                    if (allDone) setTimeout(() => _tryAutoGenerate(), 100);
                }
            }
