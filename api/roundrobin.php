...
function initTournament() {
    ...
    // 試合開始でラウンド番号が1に戻るため、自動生成の重複防止状態も初期化する
    _resetAutoGenTracking();
...
function _resetState() {
    ...
    _resetAutoGenTracking(); // ラウンド番号が1に戻るため重複防止状態も初期化
...
function generateCourtsRating(ids) {
    ...
    if (muRange < 1.0) {
        ...
        if (pairs && pairs.length > 0) {
            const courts = assignCourtsRandom(pairs);
            if (courts && courts.length > 0) return courts;
        }
        // フォールバック: assignCourtsRandom が空を返した場合も findBestCourtGroups で対処
    }

    // ② μ値が近い4人を1コートグループとして抽出
    const bestGroups = findBestCourtGroups(ids, courtCount);
    if (!bestGroups || bestGroups.length === 0) return null;
...
let _lastAutoGeneratedRound = 0;

// 組合せを削除・リセットしたときに自動生成の重複防止状態を初期化する。
// _lastAutoGeneratedRound は単調増加のため、ラウンドを消して番号が振り直されると
// 次の targetRound がこの値以下になり、自動生成がガードに弾かれ続けてしまう
// （組合せを全消しした後、2周目以降は次の試合が自動で作られなくなる）。
// done 検出済みの mid も同時に破棄する。
function _resetAutoGenTracking() {
    _lastAutoGeneratedRound = 0;
    if (window._fbResetDoneTracking) window._fbResetDoneTracking();
}

// 自動組合せトリガー（重複防止ガード付き）
// _scoresRef から呼び出す。同一インスタンム内および複数インスタンス間の二重生成を防止する。
function _tryAutoGenerate() {
    if (!isAdmin) return;
    if (!state.autoMatch) return;
    const targetRound = state.roundCount + 1;
    // ① 同一インスタンス内の重複呼び出し防止
    if (_lastAutoGeneratedRound >= targetRound) return;
    // ② 別インスタンスが既に生成済みの場合（_fbApply で schedule が更新されている）
    if (state.schedule.some(r => r.round === targetRound)) return;
    _lastAutoGeneratedRound = targetRound;
    const ok = generateNextRound();
    // 生成失敗（人数不足など）の場合はフラグを戻して次回の再試行を可能にする
    if (!ok) _lastAutoGeneratedRound = targetRound - 1;
}
...
function generateNextRound() {
    ...
    // 重複生成防止: このラウンドが既にスケジュールに存在する場合はスキップ
    const roundNum = state.roundCount + 1;
    if (state.schedule.some(r => r.round === roundNum)) {
        console.warn('[generateNextRound] round', roundNum, 'already exists, skipping duplicate generation');
        return false;
    }
...
function onNextRoundBtn() {
    if (state.autoMatch && state.seqMatch && state.schedule.length > 0) {
        ...
        // 使用中でも割り当て済みでもない空きコート（終了済み）が1つでもあるか確認
...
function _recalcIsOnCourt() {
    ...
    if (state.seqMatch) {
        // 順次モード: 物理コートごとに最新（ラウンド番号が最大）の未終了試合のみを isOnCourt=true にする
        // 古いラウンドの未スコア試合が残っていても二重カウントしない
        const latestByPhys = {}; // physIdx -> { round, courtObj }
        state.schedule.forEach(rd => {
            rd.courts.forEach((ct, ci) => {
                const physIdx = ct.physicalIndex !== undefined ? ct.physicalIndex : ci;
                const existing = latestByPhys[physIdx];
                if (!existing || rd.round > existing.round) {
                    latestByPhys[physIdx] = { round: rd.round, ci, ct };
                }
            });
        });
        Object.values(latestByPhys).forEach(({ round, ci, ct }) => {
            const sc = state.scores[`r${round}c${ci}`];
            if (!sc?.done) {
        ...
        // 一括モード: 全ラウンドの非終了コートを isOnCourt=true
        state.schedule.forEach(rd => {
            rd.courts.forEach((ct, ci) => {
...
function updatePoolStatus() {
    ...
    if (state.seqMatch) {
        bar.style.display = '';
        const avail = state.players.filter(p => !p.resting && !p.isOnCourt).length;
        const resetBtn = `<button onclick="resetPoolAndRegen()" style="margin-left:8px;padding:2px 8px;font-size:0.75rem;background:#546e7a;color:#fff;border:none;border-radius:4px;cursor:pointer;">🔄 プール再生成</button>`;
        bar.innerHTML = `🗂 プール: ${state.matchPool.length} 試合待機中（待機中: ${avail}人）${resetBtn}`;
...
function resetPoolAndRegen() {
    ...
    if (generatePoolBatch()) {
        saveState();
        showToast('✅ プールを再生成しました');
    } else {
        const avail = state.players.filter(p => !p.resting && !p.isOnCourt).length;
        showToast(`⚠️ プール再生成失敗: 利用可能 ${avail}人`);
    }
    updatePoolStatus();
}
...
function markCourtDone(roundNum, courtIndex) {
    ...
    // この画面で done を処理済みとして登録する。
    // scores リスナーは _cid チェックを迂回するため、登録しないと自分の書き込みが
    // エコーで返ってきたときに同じ done で二重に組合せが作られる。
    if (window._fbMarkDoneProcessed) window._fbMarkDoneProcessed(mid);
    ...
    if (state.autoMatch) {
        if (state.seqMatch) {
            ...
            if (rd) {
                ...
                if (allDone) _tryAutoGenerate(); // 重複防止ガード付き
            }
        }
    }
...
function markRoundDone(e, roundNum) {
    ...
    // 全コートをdone（エコーでの二重生成を防ぐため処理済みとして登録する）
    rd.courts.forEach((ct, ci) => {
    ...
    saveState();
    renderMatchContainer();
    // 自動ONの場合のみ次のラウンドを自動生成（重複防止ガード付き）
...
function generatePoolBatch() {
    ...
    const active = state.players.filter(p => !p.resting);
    if (active.length < 4) {
        console.warn('[generatePoolBatch] active < 4:', active.length, 'tempResting:', tempResting.length);
    ...
    try {
        if (state.matchingRule === 'rating') {
            ...
            if (!ids || ids.length < 4) {
                console.warn('[generatePoolBatch] selectRoundPlayers < 4:', ids?.length,
                    'active:', active.length, 'tempResting:', tempResting.length);
                restore(); return false;
            }
            courts = generateCourtsRating(ids);
    ...
    if (!courts || courts.length === 0) {
        console.warn('[generatePoolBatch] courts empty after generation, matchingRule:', state.matchingRule);
        return false;
    }
...
function assignNextPoolMatch(fromPhysicalIndex) {
    ...
    if (fromPhysicalIndex === undefined) {
        ...
        // 現在使用中（未終了＝呼び出し中・試合中）の物理コートを特定
        // 'calling'（呼び出し中）も使用中として扱い、終了済みコートのみを空きとみなす
        const inProgressPhy = new Set();
        state.schedule.forEach(rd => {
            rd.courts.forEach((ct, ci) => {
                const mid = `r${rd.round}c${ci}`;
        ...
        if (canAdd) {
            ...
            // 新しいラウンドを開始 → 使用中でない（終了済み）最初のコートから
            fromPhysicalIndex = -1;
            for (let i = 0; i < (state.courts || 2); i++) {
    ...
    if (state.matchPool.length === 0) {
        if (!generatePoolBatch()) {
            // 1回目失敗: isOnCourt を再計算してリトライ（stale フラグ対策）
            _recalcIsOnCourt();
            if (!generatePoolBatch()) {
                const avail = state.players.filter(p => !p.resting && !p.isOnCourt).length;
                const need  = Math.min(state.players.filter(p => !p.resting).length, state.courts * 4);
                showToast(`⚠️ 次の組合せの生成に失敗しました（利用可能: ${avail}人 / 必要: ${Math.floor(need/4)*4}人）`);
                return;
            }
        }
    }
...
function deleteRound(e, roundNum) {
    ...
    // ラウンド番号を詰め直したので自動生成の重複防止状態を初期化する
    // （旧 r3c0 が新 r2c0 になるため、古い mid が残ると自動割当が発火しなくなる。
    //   _lastAutoGeneratedRound も戻さないと2周目以降の自動生成が止まる）
    _resetAutoGenTracking();
...
function deleteCallingCourt(roundNum, courtArrayIdx) {
    ...
    _recalcIsOnCourt();
    // ラウンド番号を詰め直した可能性があるため自動生成の重複防止状態を初期化
    _resetAutoGenTracking();
    state.matchPool = [];
    saveState();
    ...
    try {
        ...
        Object.keys(remoteState.scores || {}).forEach(mid => {
            ...
            if (s && (s.pt1 !== undefined || s.pt2 !== undefined)) {
                if (!window._livePtScores[mid]) window._livePtScores[mid] = {};
                if (s.pt1 !== undefined) window._livePtScores[mid].pt1 = s.pt1;
                if (s.pt2 !== undefined) window._livePtScores[mid].pt2 = s.pt2;
            }
            // done になったコートはキャッシュをクリア（試合終了後は不要）
            if (remoteState.scores[mid]?.done) delete window._livePtScores[mid];
        });
...
// immediate=true: デバウンスなしで即時 Firebase push（ペア固定・休憩設定など競合敏感な操作に使用）
function saveState(immediate = false) {
    ...
    if (!isApplyingRemote && window._fbPush) {
        clearTimeout(_fbPushTimer);
        if (immediate) {
            // 即時送信: 300ms以内に_fbApplyが別クライアント更新を上書きするのを防ぐ
            window._fbPush(state);
        } else {
...
// scores リスナーは _cid チェックを迂回してエコーも受け取るため、これを呼ばないと
// 「試合終了」ボタンで自分が組合せを作った直後に、返ってきたエコーで
// もう一度組合せが作られてしまう。
window._fbMarkDoneProcessed = function(mid) {
    if (mid) _processedDone.add(mid);
};
...
window._fbStart = function(sessionId) {
    ...
    onValue(_scoresRef, snap => {
        ...
        // _ref（親パス）が _scoresRef（子パス）より先に発火した場合、_fbApply が
        // state.scores[mid].done=true を先に書いてしまい alreadyDone=true になる問題を回避。
        if (isAdmin && (state.autoMatch || state.seqMatch) && Array.isArray(state.schedule)) {
            state.schedule.forEach(rd => {
                (rd.courts || []).forEach((ct, ci) => {
                    const mid = 'r' + rd.round + 'c' + ci;
                    const nowDone = !!scores[mid]?.done;
                    if (nowDone && !_processedDone.has(mid) && !_pendingAssign.has(mid)) {
                        _processedDone.add(mid); // 処理済みとしてマーク（以降のエコーで再トリガーしない）
                        // isOnCourt を解放
                        [...(ct.team1 || []), ...(ct.team2 || [])].forEach(id => {
                            const p = state.players.find(pp => pp.id === id);
                            if (p) p.isOnCourt = false;
...
// _livePtScores は _fbApply（メインスクリプト）からも参照するため window プロパティとして公開する。
