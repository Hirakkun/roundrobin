...
import { getDatabase, ref, get, remove, child }
...
function buildCard(s) {
    ...
    const createdAt = d.createdAt
        ? new Date(d.createdAt).toLocaleString('ja-JP', { year:'numeric', month:'2-digit', day:'2-digit', hour:'2-digit', minute:'2-digit' })
        : '—';
...
window.toggleCard = function(fbKey) {
    ...
    const detail = document.getElementById('detail-' + CSS.escape(fbKey));
...
window.executeDelete = async function() {
    ...
    if (ng === 0) {
        showToast(`✅ ${ok} 件削除しました`);
    } else {
        showToast(`⚠️ ${ok} 件削除、${ng} 件失敗`);
    }
    await loadSessions();
};
